"""Support for Junctek ble sensors."""
from __future__ import annotations

import logging

from junctek_ble import JunctekDevice
...
async def async_setup_entry(
    ...
    """Set up the Junctek BLE sensors."""
    is_metric = hass.config.units is METRIC_SYSTEM

    coordinator: DataUpdateCoordinator[JunctekDevice] = hass.data[DOMAIN][
        entry.entry_id
    ]
    ...
    for sensor_type, sensor_value in coordinator.data.sensors.items():
        ...
        entities.append(
            JunctekSensor(coordinator, coordinator.data, sensors_mapping[sensor_type])
        )
...
class JunctekSensor(
    CoordinatorEntity[DataUpdateCoordinator[JunctekDevice]], SensorEntity
):
    """Junctek BLE sensors for the device."""

    _attr_has_entity_name = True

    def __init__(
        self,
        coordinator: DataUpdateCoordinator[JunctekDevice],
        junctek_device: JunctekDevice,
        entity_description: SensorEntityDescription,
    ) -> None:
        """Populate the Junctek entity with relevant data."""
        super().__init__(coordinator)
        self.entity_description = entity_description

        name = junctek_device.name
        if identifier := junctek_device.identifier:
            name += f" ({identifier})"

        self._attr_unique_id = f"{junctek_device.address}_{entity_description.key}"
        self._attr_device_info = DeviceInfo(
            connections={
                (
                    CONNECTION_BLUETOOTH,
                    junctek_device.address,
                )
            },
            name=name,
            manufacturer=junctek_device.manufacturer,
            hw_version=junctek_device.hw_version,
            sw_version=junctek_device.sw_version,
            model=junctek_device.model,
        )
